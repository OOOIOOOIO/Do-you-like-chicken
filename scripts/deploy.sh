...
JAR_NAME=$(ls -tr $REPOSITORY/dev | grep jar | tail -n 1)
...
nohup java -jar \
        -Dspring.config.location=classpath:/application.yml,/home/ubuntu/app/chicken/application-real.yml \
        $REPOSITORY/dev/$JAR_NAME > $REPOSITORY/dev/nohup.out 2>&1 &
